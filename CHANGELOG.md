...
- **移除热门标签栏 UI** — 彻底删除了工作空间过滤栏的热门标签/常用标签芯片展示，简化了多维度过滤结构，避免与搜索框、状态下拉菜单产生逻辑冲突和功能冗余。
...
- **分离状态与普通标签** — 在文件详情编辑器、列表行、网格卡片中彻底对状态型标签（如 `#待处理`）与普通内容分类标签进行分拆和样式区分展示，避免逻辑属性冲突。
- **规范新建标签逻辑** — 限制新建自定义分类标签输入框中不允许手动输入状态标签，以保证标签分类纯净。
- **仪表盘标签云过滤** — 看板页面的数据标签云图剔除了状态型标签，真实并直观地反映文件具体内容的类目分布。
